#!/usr/bin/env python3
"""
Translate Hindi plain-text files to low-resource languages using Gemini with context caching.

This script:
1. Uses monolingual examples for Gemini's context caching
...
5. Uses FREE Gemini 2.0 Flash Experimental by default

Usage:
    # Free translation with Gemini 2.0 Flash
    python scripts/translate_hindi_to_lowres_gemini.py --target-lang mwr
    python scripts/translate_hindi_to_lowres_gemini.py --target-lang awa --dry-run
    python scripts/translate_hindi_to_lowres_gemini.py --target-lang brx --max-files 5

    # Use paid Gemini model
    python scripts/translate_hindi_to_lowres_gemini.py --target-lang mai --model gemini-1.5-flash
"""
...
from universal_translate.providers.gemini_provider import GeminiProvider
...
class DirectoryTranslator:
    ...
    def translate_directory(
        ...
        """
        print("="*70)
        print("GEMINI DIRECTORY TRANSLATION")
        print("="*70)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Translate Hindi to low-resource languages using Gemini with context caching"
    )
    ...
    parser.add_argument(
        ...
        help="Gemini model to use (default: gemini-2.0-flash-exp FREE)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of sentences per API call (default: 20, higher for Gemini)"
    )
    ...
    # Create provider
    print(f"\nInitializing Gemini provider ({args.model})...")

    # Note about free tier
    if args.model == 'gemini-2.0-flash-exp':
        print("  ✨ Using FREE Gemini 2.0 Flash Experimental!")
    ...
    if args.estimate_only or args.dry_run:
        ...
        print(f"  Subsequent: ~75% savings on cached content (Gemini)")

        if args.model == 'gemini-2.0-flash-exp':
            print(f"\n  💰 Total estimated cost: FREE (Gemini 2.0 Flash Experimental)")
