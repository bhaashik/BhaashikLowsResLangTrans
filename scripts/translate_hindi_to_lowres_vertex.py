#!/usr/bin/env python3
"""
Translate Hindi plain-text files to low-resource languages using Vertex AI (Enterprise Gemini).

This script:
1. Uses monolingual examples for Gemini's context caching on Vertex AI
2. Translates Hindi files in input/converted/Hindi/plain-text/by_file/
3. Recreates directory structure in output directory
4. Supports all 6 low-resource languages (Marwari, Awadhi, Braj, Bodo, Maithili, Nepali)
5. Uses enterprise Gemini on GCP with SLA and 50% cost savings vs Gemini API
...
Usage:
    # Enterprise translation with Vertex AI
    python scripts/translate_hindi_to_lowres_vertex.py --target-lang mwr
    python scripts/translate_hindi_to_lowres_vertex.py --target-lang awa --dry-run
    python scripts/translate_hindi_to_lowres_vertex.py --target-lang brx --max-files 5

    # Use specific Gemini model on Vertex
    python scripts/translate_hindi_to_lowres_vertex.py --target-lang mai --model gemini-1.5-pro-002
"""
...
from universal_translate.providers.vertex_provider import VertexAIProvider
...
class DirectoryTranslator:
    ...
    def translate_directory(
        ...
        """
        print("="*70)
        print("VERTEX AI DIRECTORY TRANSLATION")
        print("="*70)
        print(f"Input:  {self.input_base}")
        print(f"Output: {self.output_base}")
        print(f"Languages: {self.src_lang} → {self.target_lang}")
        print(f"Model: {self.provider.model}")
        print(f"Project: {self.provider.project_id}")
        print(f"Location: {self.provider.location}")
    ...
    def _translate_file(self, input_file: Path, output_file: Path, batch_size: int):
        ...
        for batch_start in range(0, len(sentences), batch_size):
            ...
            # Cost display (INR for Vertex AI)
            cost_inr = response.total_cost
            cost_usd = cost_inr / 85  # Approximate conversion
            print(f"Done (cost: ₹{cost_inr:.4f} / ${cost_usd:.4f})")
            self.stats['total_cost'] += cost_inr
    ...
    def _print_summary(self):
        ...
        print(f"Total cost:            ₹{self.stats['total_cost']:.2f} INR (${self.stats['total_cost']/85:.2f} USD)")

        if self.stats['sentences_translated'] > 0:
            avg_cost = self.stats['total_cost'] / self.stats['sentences_translated']
            print(f"Average cost/sentence: ₹{avg_cost:.6f} (${avg_cost/85:.6f} USD)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Translate Hindi to low-resource languages using Vertex AI (Enterprise Gemini)"
    )

    parser.add_argument(
        '--target-lang',
        required=True,
        choices=['mwr', 'awa', 'bra', 'brx', 'mai', 'nep', 'bho', 'mag'],
        help="Target language (mwr=Marwari, awa=Awadhi, bra=Braj, brx=Bodo, mai=Maithili, nep=Nepali)"
    )

    parser.add_argument(
        '--model',
        default='gemini-1.5-flash-002',
        help="Vertex AI model to use (default: gemini-1.5-flash-002)"
    )
    ...
    # Load prompt configuration (prefer Gemini config)
    prompt_config = base_dir / f"universal_translate/config/prompts/hi_to_{args.target_lang}_gemini.yaml"

    # Fallback to OpenAI config if Gemini-specific not found
    if not prompt_config.exists():
        print(f"ℹ Gemini config not found, trying OpenAI config...")
    ...
    # Create provider
    print(f"\nInitializing Vertex AI provider ({args.model})...")
    ...
    try:
        ...
        print(f"  ✓ Vertex AI configured (Enterprise SLA)")
    except Exception as e:
    ...
    if args.estimate_only or args.dry_run:
        ...
        units = [TranslationUnit(text="x" * avg_chars, index=0)] * total_sentences
        ...
        print(f"  Estimated input cost: ₹{estimate.input_cost:.2f} (${estimate.input_cost/85:.2f})")
        print(f"  Estimated output cost: ₹{estimate.output_cost:.2f} (${estimate.output_cost/85:.2f})")
        print(f"  Total estimated cost: ₹{estimate.total_cost:.2f} INR (${estimate.total_cost/85:.2f} USD)")

        if total_sentences > 0:
            print(f"  Cost per sentence: ₹{estimate.total_cost/total_sentences:.6f} (${estimate.total_cost/total_sentences/85:.6f})")
        ...
        print(f"  Subsequent: ~75% savings on cached content")

        print(f"\n  💼 Vertex AI benefits: Enterprise SLA, 50% cheaper than Gemini API")
